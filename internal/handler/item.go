...
func (h *Handler) allItems(c *gin.Context) {
	userId, err := h.getUserId(c)
	if err != nil {
		newErrorResponse(c, http.StatusUnauthorized, "Invalid User")
	}
	...
	items, err := h.services.TodoItem.All(userId, listId)
	if err != nil {
		newErrorResponse(c, http.StatusInternalServerError, err.Error())
	}
...
func (h *Handler) item(c *gin.Context) {
	...
	item, err := h.services.TodoItem.ItemByID(userId, itemId)
	if err != nil {
		newErrorResponse(c, http.StatusInternalServerError, err.Error())
		return
	}
